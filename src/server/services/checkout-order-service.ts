...
import {
  canAccessOrder,
  rememberGuestOrder,
} from "@/server/services/order-access-service";
  ...
  try {
    ...
    if (existing) {
      if (!(await canAccessOrder(existing)))
        throw new ActionError("Pedido no encontrado.");
      return {
        ok: true,
    ...
    const order = await db.$transaction(async (tx) => {
      ...
      const created = await tx.order.create({
        data: {
          ...
          accountId: customerSession?.user.id ?? null,
          email: data.email.toLowerCase(),
          phone: data.phone,
          firstName: data.firstName,
          lastName: data.lastName,
    ...
    if (
      ...
      if (dup && (await canAccessOrder(dup))) {
        return {
          ok: true,
